...
const Artist = () => {
  const { artists, loading, totalPages, fetchArtists, addArtist, updateArtistInfo, removeArtist, error } = useArtists();
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "first_release_year" || name === "no_of_albums_released" ? Number(value) : value,
    }));
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white p-6 rounded-xl shadow-sm gap-4 border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage artist profiles, albums, and CSV data.</p>
        </div>
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          ...
          />
          <Button variant="secondary" onClick={handleImport} className="flex gap-2 flex-1 sm:flex-none">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
            </svg>
            Import CSV
          </Button>
          <Button variant="secondary" onClick={handleExport} className="flex gap-2 flex-1 sm:flex-none">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
            </svg>
            Export CSV
          </Button>
          <Button onClick={handleCreate} className="flex gap-2 flex-1 sm:flex-none">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Table
        headers={["Artist Name", "DOB", "Gender", "Address", "First Release", "Albums", "Actions"]}
        pagination={{
          currentPage,
      ...
      >
        {loading ? (
          ...
          artists.map((artist) => (
            <tr key={artist._id} className="hover:bg-gray-50 transition border-b border-gray-100 last:border-0">
              <td className="px-6 py-4 font-semibold text-purple-900">{artist.name}</td>
              <td className="px-6 py-4 text-gray-600">
                {artist.dob ? new Date(artist.dob).toLocaleDateString() : "-"}
              </td>
              <td className="px-6 py-4 capitalize text-gray-600">{artist.gender || "-"}</td>
              <td className="px-6 py-4 text-gray-600 truncate max-w-[150px]">{artist.address || "-"}</td>
              <td className="px-6 py-4 text-gray-600">{artist.first_release_year || "-"}</td>
              ...
              <td className="px-6 py-4">
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/artist/${artist._id}`)} className="text-purple-700 hover:text-purple-900 hover:bg-purple-50">
                    View Songs
                  </Button>
                  <div className="h-6 w-px bg-gray-200 self-center mx-1"></div>
                  <Button variant="secondary" size="sm" onClick={() => handleEdit(artist)}>
                    Edit
                  </Button>
                  <Button variant="danger" size="sm" onClick={() => handleDelete(artist._id!)}>
                    Delete
                  </Button>
                </div>
              </td>
      ...
      >
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-700">First Release Year</label>
              <input
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-700">Number of Albums</label>
              <input
          ...
          <div className="pt-6 flex flex-col-reverse sm:flex-row gap-3 justify-end border-t mt-6">
            ...
            <Button type="submit" className="w-full sm:w-auto" disabled={loading}>
              {loading ? "Processing..." : editingArtist ? "Save Changes" : "Create Artist"}
            </Button>
          </div>
        </form>
      </Modal>
